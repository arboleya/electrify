...
App.prototype.start_meteor = function(done){
  ...
  this.meteor_process = spawn(command, [], {
    ...
    // not good mr windows
    stdio: this.$.env.sys.is_windows ? 'pipe' : 'inherit'
  });
...
App.prototype.ensure_deps = function(done) {
  ...
  // in development mode, override install with local electrify version
  if(this.$.env.is_development_mode) {

    var self_path = join(__dirname, '..'); 
    var npm = 'npm' + (this.$.env.sys.is_windows ? '.bat' : '');
    spawn(npm, ['install', '--save', self_path], {
