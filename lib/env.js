...
module.exports = function(input, output, config, settings, cli) {
  return new Env(input, output, config, settings, cli);
};

function Env(input, output, config, settings, cli) {
  // shortcuts
  var join = path.join;
  ...
  if(!fs.existsSync(path.join(this.app.root, '.meteor'))) {
  ...
  this.app.config_path = config || join(this.app.electrify, 'electrify.json');
