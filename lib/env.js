...
module.exports = function(input, output, settings) {
  return new Env(input, output, settings);
};
...
function Env(input, output, settings) {
  ...
  if(!fs.existsSync(this.app.root)) {
    error_msg = 'Input folder doesn\'t exist: ' + this.app.root;
    throw new Error(error_msg);
  }
  
  // validates meteor project
  else if(!fs.existsSync(path.join(this.app.root, '.meteor'))) {
    error_msg = 'Not a meteor app: ' + this.app.root;
    throw new Error(error_msg);
  }
  ...
  this.app.dist        = output || join(this.app.electrify, '.dist');
  this.app.settings    = settings || {};

  if(!fs.existsSync(this.app.dist)) {
    error_msg = 'Output folder doesn\'t exist: ' + this.app.dist;
    throw new Error(error_msg);
  }
