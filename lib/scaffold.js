...
Scaffold.prototype.prepare = function() {
  ...
  if(!fs.existsSync(index) || this.$.env.in_development_mode) {
    ...
    fs.writeFileSync(index, [
      ...
      "",
      "",
      "app.on('will-quit', terminate_and_quit);",
      "",
      "",
      "function terminate_and_quit(event) {",
      "  // if electrify is up, wait a little with `preventDefault`, terminate",
      "  // electrify and then re-call `terminate_and_quit`",
      "  if(electrify.isup() && event) {",
      "    event.preventDefault();",
      "    electrify.stop(terminate_and_quit);       //~> electrify stop",
      "  }",
      "  // otherwise (if electrify is down), move on and quit the app",
      "}"
    ].join('\r'));
  }
