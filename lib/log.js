...
Log.prototype.info = function(){
  if(/INFO|ALL/i.test(process.env.LOGELECTRIFY))
    log(this.prefix, 'INFO', slice(arguments));
};

Log.prototype.warn = function(){
  if(/WARN|ALL/i.test(process.env.LOGELECTRIFY))
    log(this.prefix, 'WARN', slice(arguments));
};

Log.prototype.error = function(){
  if(/ERROR|ALL/i.test(process.env.LOGELECTRIFY))
    log(this.prefix, 'ERROR', slice(arguments));
};

Log.prototype.debug = function(){
  if(/DEBUG|ALL/i.test(process.env.LOGELECTRIFY))
    log(this.prefix, 'DEBUG', slice(arguments));
};

Log.prototype.trace = function(){
  if(/TRACE|ALL/i.test(process.env.LOGELECTRIFY))
