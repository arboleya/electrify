...
module.exports = function(input, output, config, settings, cli){
  return new Electrify(input, output, config, settings, cli);
};

function Electrify(input, output, config, settings, cli) {
  ...
  this.env = require('./env')(input, output, config, settings, cli);
