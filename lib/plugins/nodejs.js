...
NodeJS.prototype.start = function(done) {
  ...
  this.$.freeport(null, function(meteor_port){
    ...
    self.process = spawn(self.app_node_path, [self.meteor_main], {
      ...
      // spawned. also, no error is risen. it was discovered with try'n'error
      // so we listen for stdout and stderr instead, and things appear to be
      // working again
    });

    self.process.stdout.on('data', function(d){
      // mimics inherit
      self.log.info('meteor:stdout', d.toString());
    });

    self.process.stderr.on('data', function(d){
      // mimics inherit
      self.log.error('meteor:stderr', d.toString());
    });
