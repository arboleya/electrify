...
MongoDB.prototype.start = function(done) {
  ...
  this.$.freeport(null, function(port){
    ...
    self.pkgObj       = require(self.$.env.app.config_path) || {};
    if (self.pkgObj.dbInUserDataPath){
      if(this.$.env.sys.is_windows) self.dbdir = join(process.env.APPDATA,self.pkgObj.name);
      if(this.$.env.sys.is_osx)     self.dbdir = join(process.env.HOME,'Library','Preference',self.pkgObj.name);
      if(this.$.env.sys.is_linux)   self.dbdir = join(process.env.HOME,'var','local',self.pkgObj.name);
    }
    // ----------------------------------------------
