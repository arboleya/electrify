...
MongoDB.prototype.start = function(done) {
  ...
  this.$.freeport(null, function(port){
    ...
    self.configObj = require(self.$.env.app.config_path) || {};
    self.pkgObj    = require(join(self.$.env.app.root, 'package.json')) || {};
    if (self.configObj.dbInUserDataPath && self.pkgObj.name){
      if(self.$.env.sys.is_windows) self.dbdir = join(process.env.APPDATA,self.pkgObj.name);
      if(self.$.env.sys.is_osx)     self.dbdir = join(process.env.HOME,'Library','Preference',self.pkgObj.name);
      if(self.$.env.sys.is_linux)   self.dbdir = join(process.env.HOME,'var','local',self.pkgObj.name);
    }
    // ----------------------------------------------
