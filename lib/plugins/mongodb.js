...
MongoDB.prototype.start = function(done) {
  ...
  this.$.freeport(null, function(port){
    ...
    self.dbInUserData  = require(join(self.$.env.app.root, 'package.json')).dbInUserData || false;
    if (self.dbInUserData){
      self.dbdir = process.env.APPDATA || (process.platform == 'darwin' ? process.env.HOME + 'Library/Preference' : '/var/local')
    }
